...
class MultiAgentBaseEnv(gym.Env):
    ...
    def _set_action(self, action, policy_u, agent: Agent, action_space, time: Optional = None) -> None:
        ...
        if isinstance(action_space, MultiDiscrete):
            ...
            action_index = int(np.argmax(action)) if np.ndim(action) else int(action)
            action = [action_index]
        else:
            action = [action]
        ...
        if agent.movable:
            ...
            # Guide actions are auxiliary targets only. The environment always
            # executes the sampled actor action so PPO log-probs stay on-policy.
            agent.action.u = limit_action_inf_norm(network_output, 1)
