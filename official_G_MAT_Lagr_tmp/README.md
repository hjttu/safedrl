...
## Graph-Embedded CBF Shield

`GSMPE` uses a joint `20 x 20` discrete acceleration table and applies an
analytical DTCBF mask before every actor sample. A small graph-conditioned
safety head ranks only the actions that remain in the analytical safe set.
Training also includes a safe-guide imitation loss with sigmoid decay and an
MSE safety-ranking loss.

The guide policy is never executed in place of the actor action. This keeps the
environment action consistent with the PPO log probability. Because the action
