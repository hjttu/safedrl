...
class GS_MAPPO():
    ...
    def ppo_update(self, 
        ...
        aver_episode_costs = base_sample
        actions_rl_cont_batch = actions_safe_cont_batch = cbf_diag_batch = None
        if len(sample) >= 25:
            actions_rl_cont_batch, actions_safe_cont_batch, cbf_diag_batch = sample[22:25]
        ...
        int_loss = torch.zeros((), **self.tpdv)
        trm_loss = torch.zeros((), **self.tpdv)
        if actions_rl_cont_batch is not None and self.lambda_int > 0:
            actions_rl_cont_batch = check(actions_rl_cont_batch).to(**self.tpdv)
            actions_safe_cont_batch = check(actions_safe_cont_batch).to(**self.tpdv)
            int_per_sample = torch.mean((actions_safe_cont_batch - actions_rl_cont_batch) ** 2, dim=-1, keepdim=True)
            int_loss = torch.mean(int_per_sample)
            policy_loss = policy_loss + self.lambda_int * torch.mean(imp_weights * int_per_sample)
        if cbf_diag_batch is not None and self.lambda_trm > 0:
            cbf_diag_batch = check(cbf_diag_batch).to(**self.tpdv)
            trm_per_sample = cbf_diag_batch[:, 4:5] * cbf_diag_batch[:, 5:6]
            trm_loss = torch.mean(trm_per_sample)
            policy_loss = policy_loss + self.lambda_trm * torch.mean(imp_weights * trm_per_sample)
        ...
        return (value_loss, critic_grad_norm, policy_loss, dist_entropy, 
                ...
                int_loss.detach(), trm_loss.detach())
    ...
    def train(self, 
        ...
        train_info['int_loss'] = 0
        train_info['trm_loss'] = 0
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                train_info['int_loss'] += int_loss.item()
                train_info['trm_loss'] += trm_loss.item()
