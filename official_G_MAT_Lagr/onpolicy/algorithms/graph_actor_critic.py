...
class GR_Actor(nn.Module):
    ...
    def __init__(
        ...
        self.use_attention_priority = getattr(args, "use_attention_priority", False)
        self.use_responsibility = getattr(args, "use_responsibility", False)
        edge_head_hidden = max(16, args.gnn_hidden_size)
        self.priority_head = nn.Sequential(
            nn.Linear(edge_dim, edge_head_hidden),
            nn.ReLU(),
            nn.Linear(edge_head_hidden, 1),
            nn.Sigmoid(),
        )
        self.responsibility_head = nn.Sequential(
            nn.Linear(edge_dim, edge_head_hidden),
            nn.ReLU(),
            nn.Linear(edge_head_hidden, 1),
            nn.Sigmoid(),
    ...
    def safety_edge_scores(self, adj):
        ...
        if self.use_responsibility:
            ...
            if edge_attr.shape[-1] > 9:
                entity_type_j = edge_attr[..., 9].round().long()
                agent_agent = entity_type_j.eq(0)
                denom = gamma_raw + gamma_raw.transpose(1, 2) + 1e-6
                gamma = gamma_raw / denom
                gamma = torch.where(agent_agent, gamma, torch.ones_like(gamma))
            gamma = gamma * edge_mask
        else:
            gamma = edge_mask
        return priority, gamma
