...
class SimplifiedAttentionNet(nn.Module):
    ...
    def process_adj(adj: Tensor, max_edge_dist: float) -> Tuple[Tensor, Tensor]:
        ...
        """
        assert adj.dim() >= 2 and adj.dim() <= 4
        if adj.dim() in (3, 4) and adj.size(-1) != adj.size(-2):
            dist_adj = adj[..., 0]
            full_edge_attr = adj
        else:
            dist_adj = adj
            full_edge_attr = None
        assert dist_adj.size(-1) == dist_adj.size(-2)

        # filter far away nodes and connection to itself
        connect_mask = ((dist_adj < max_edge_dist) & (dist_adj > 0)).float()
        dist_adj = dist_adj * connect_mask
        if dist_adj.dim() == 3:
            # Case: (batch_size, num_nodes, num_nodes)
            batch_size, num_nodes, _ = dist_adj.shape
            edge_index = dist_adj.nonzero(as_tuple=False)
            if full_edge_attr is None:
                edge_attr = dist_adj[edge_index[:, 0], edge_index[:, 1], edge_index[:, 2]]
            else:
                edge_attr = full_edge_attr[edge_index[:, 0], edge_index[:, 1], edge_index[:, 2], :]
            ...
            edge_index = dist_adj.nonzero(as_tuple=False).t().contiguous()
            if full_edge_attr is None:
                edge_attr = dist_adj[edge_index[0], edge_index[1]]
            else:
                edge_attr = full_edge_attr[edge_index[0], edge_index[1], :]
