...
class GSReplayBuffer(object):
    ...
    def __init__(
        ...
        self.actions_rl_cont = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, 2),
            dtype=np.float32,
        )
        self.actions_safe_cont = np.zeros_like(self.actions_rl_cont)
        self.cbf_diag = np.zeros(
    ...
    def insert(
        ...
        if actions_rl_cont is not None:
            self.actions_rl_cont[self.step] = actions_rl_cont.copy()
        if actions_safe_cont is not None:
    ...
    def feed_forward_generator(
        ...
        actions_rl_cont = self.actions_rl_cont.reshape(-1, 2)
        actions_safe_cont = self.actions_safe_cont.reshape(-1, 2)
        cbf_diag = self.cbf_diag.reshape(-1, self.cbf_diag.shape[-1])
        if self.available_actions is not None:
        ...
        for indices in sampler:
            ...
            actions_rl_cont_batch = actions_rl_cont[indices]
            actions_safe_cont_batch = actions_safe_cont[indices]
            cbf_diag_batch = cbf_diag[indices]
            if self.available_actions is not None:
            ...
            yield (
                ...
                cost_adv_targ,
                aver_episode_costs,
                actions_rl_cont_batch,
                actions_safe_cont_batch,
                cbf_diag_batch,
            )
    ...
    def naive_recurrent_generator(
        ...
        actions_rl_cont = self.actions_rl_cont.reshape(-1, batch_size, 2)
        actions_safe_cont = self.actions_safe_cont.reshape(-1, batch_size, 2)
        cbf_diag = self.cbf_diag.reshape(-1, batch_size, self.cbf_diag.shape[-1])
        if self.available_actions is not None:
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                actions_rl_cont_batch.append(actions_rl_cont[:, ind])
                actions_safe_cont_batch.append(actions_safe_cont[:, ind])
                cbf_diag_batch.append(cbf_diag[:, ind])
                if self.available_actions is not None:
            ...
            actions_rl_cont_batch = np.stack(actions_rl_cont_batch, 1)
            actions_safe_cont_batch = np.stack(actions_safe_cont_batch, 1)
            cbf_diag_batch = np.stack(cbf_diag_batch, 1)
            if self.available_actions is not None:
            ...
            actions_rl_cont_batch = _flatten(T, N, actions_rl_cont_batch)
            actions_safe_cont_batch = _flatten(T, N, actions_safe_cont_batch)
            cbf_diag_batch = _flatten(T, N, cbf_diag_batch)
            if self.available_actions is not None:
    ...
    def recurrent_generator(
        ...
        actions_rl_cont = _cast(self.actions_rl_cont)
        actions_safe_cont = _cast(self.actions_safe_cont)
        cbf_diag = _cast(self.cbf_diag)
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                actions_rl_cont_batch.append(actions_rl_cont[ind : ind + data_chunk_length])
                actions_safe_cont_batch.append(actions_safe_cont[ind : ind + data_chunk_length])
                cbf_diag_batch.append(cbf_diag[ind : ind + data_chunk_length])
                if self.available_actions is not None:
            ...
            actions_rl_cont_batch = np.stack(actions_rl_cont_batch, axis=1)
            actions_safe_cont_batch = np.stack(actions_safe_cont_batch, axis=1)
            cbf_diag_batch = np.stack(cbf_diag_batch, axis=1)
            if self.available_actions is not None:
            ...
            actions_rl_cont_batch = _flatten(L, N, actions_rl_cont_batch)
            actions_safe_cont_batch = _flatten(L, N, actions_safe_cont_batch)
            cbf_diag_batch = _flatten(L, N, cbf_diag_batch)
            if self.available_actions is not None:
