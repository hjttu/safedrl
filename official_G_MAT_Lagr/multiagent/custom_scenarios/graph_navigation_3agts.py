...
from onpolicy.algorithms.cbf.features import build_cbf_edge_matrix
...
class Scenario(BaseScenario):
    ...
    def make_world(self, args: argparse.Namespace) -> World:
        ...
        world.use_cbf_edge_features = getattr(args, "use_cbf_filter", False)
        world.d_safe_agent = getattr(args, "d_safe_agent", 0.25)
        world.d_safe_obstacle = getattr(args, "d_safe_obstacle", 0.30)
    ...
    def reset_world(self, world: World) -> None:
        ...
        # Use the preset scenarios first for reproducibility, then keep training
        # by sampling from all available scenarios instead of exhausting SID.
        global SID
        sid = SID.pop(0) if SID else np.random.randint(0, len(Scenarios.data))
    ...
    def graph_observation(self, agent: Agent, world: World) -> Tuple[arr, arr]:
        ...
        if getattr(world, "use_cbf_edge_features", False):
            adj = build_cbf_edge_matrix(world, world.d_safe_agent, world.d_safe_obstacle)
        else:
